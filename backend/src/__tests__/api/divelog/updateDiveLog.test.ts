...
describe('PUT /divelog/:id', () => {
    ...
    test.each(validSingleFieldUpdate)('200 with authentication and valid JSON payload', async ({ field, value }) => {
        const payload = {
            location: {
                type: "Point",
                coordinates: [40.712776, -74.005974]
    ...
    test.each(invalidUpdateCasesDiveLog)('400 for invalid update of %s', async ({ field, value, message }) => {
        const payload = {
            fishTags: [],
            location: {
                type: "Point",
