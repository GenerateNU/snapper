...
const ProfileTags: React.FC<ProfileTagProps> = ({
    id
  }) => {
    const {data, isLoading, isError} = useUserById(id);
    const { handleFollowToggle, isFollowing, isPending } = useFollow(data?.user._id);
    const uri = data?.user.profilePicture ? data?.user.profilePicture : PROFILE_PHOTO;
    const { mongoDBId } = useAuthStore()
    const isViewingOwnProfile = mongoDBId === id;
    return (
        <TouchableOpacity
            onPress={() =>  {router.push(`/user/${data?.user._id}`)}}
