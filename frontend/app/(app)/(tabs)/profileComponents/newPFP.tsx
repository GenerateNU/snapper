...
import { FieldValues, useFormContext, UseFormSetValue, useFormState } from 'react-hook-form';
        ...
        return (
            <View className={`w-fit h-fit flex justify-center items-center pb-2`}>
                ...
                <TouchableOpacity
                    ...
                    onPress={async () => { const uri = await pickImage(setValue); setImage(uri) }}>
                    {image && <Image className="absolute w-full h-full rounded-full object-cover" source={{ uri: image }} />}
                    <Image className="object-cover" source={require("../../../../assets/Plus.png")} />
                </TouchableOpacity>
            </View>
        );
    }
